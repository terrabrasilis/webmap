...
export class DeforestationOptionsUtils {
  ...
  public static setLoiNamesDownload(loi: any, loinames: any, checkedLoiNames: any) {
    ...
    loi.loinames.forEach(function(loiname: any) {
      let [mun, state] = loiname.loiname.split('_');
  ...
  public static setLoiNamesSplit(loi: any, self: any) {
    ...
    loi.loinames.forEach(function(loiname: any) {
      let value = loiname.loiname.split('_')[0];
      self.loiNames[loiname.gid] = value;
      self.loiNamesObject.push({key: loiname.gid, value});
    });
  ...
  public static dataWranglingRates(dataJson: any) {
    ...
    dataJson.periods.forEach(function(period: any) {
      period.features.forEach(function(feature: any) {
        let year = period.endDate.year;
        let area = feature.areas.filter((area: any) => area.type == 1).map(function(e: any) { return e.area; })[0];
        let filteredArea = feature.areas.filter((area: any) => area.type == 2).map(function(e: any) { return e.area; })[0];
        all.push({
          endDate: year,
          loi: feature.loi,
  ...
  public static dataWranglingIncrements(dataJson: any) {
    ...
    dataJson.periods.forEach(function(period: any) {
      period.features.forEach(function(feature: any) {
        let year = period.endDate.year;
        let area = feature.areas.filter((area: any) => area.type == 2).map(function(e: any) { return e.area; })[0];
        let filteredArea = feature.areas.filter((area: any) => area.type == 1).map(function(e: any) { return e.area; })[0];
  ...
  public static renderGraph(id: any, listCharts: any, transition: any, loiNames: any, type: any): void {
    ...
    let width = $('#' + id).width(); // get width from parent div
    let height = $('#' + id).height(); // get height from parent div

    let result = listCharts.get(id); // get result from list charts
