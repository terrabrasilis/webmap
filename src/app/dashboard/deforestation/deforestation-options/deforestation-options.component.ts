...
export class DeforestationOptionsComponent implements OnInit  {
  ...
  ngOnInit() {
    ...
    $("a.nav-link.ripple").click(function() {
      var idx = self.lois.findIndex(function(el) {return el.value == self.selectedLoi;} );
...
//    .sortBy(function(d:any) { return d['loiName']; })
  ...
  makeGraphs():void {
    ...
    var filteredAreaByLoiName = this.loiNameDim.group().reduceSum(
      function(d:any) {
        return +d["filteredArea"];
      }
    );
    ...
    var filteredAreaByloiNameYear = loiNameYearDim.group().reduceSum(function(d:any) {
			return +d["filteredArea"];
    });
    ...
    var loiNameGroup = this.loiNameDim.group().reduceCount();

    // define values (to be used in charts)
    this.minDate = dateDim.bottom(1)[0]["endDate"];
    this.maxDate = dateDim.top(1)[0]["endDate"]; 
    var maxAreaLoiName = this.areaByLoiName.top(1)[0].value;
    ...
    var loiChartWidth = $('#loi-chart')[0].offsetWidth;
    var loiChartHeight = $('#loi-chart')[0].offsetHeight;
